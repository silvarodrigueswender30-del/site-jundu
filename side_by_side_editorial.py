...
impl_path = os.path.join(base_dir, "docs", "qa", "editorial", "capture_1440px.png")
...
if os.path.exists(mockup_path) and os.path.exists(impl_path):
    ...
    # Focus on the implementation (y ~ 900 to 1900)
    w_i, h_i = impl.size
    impl_sec = impl.crop((0, min(800, h_i-10), w_i, min(1800, h_i)))
    
    combined = Image.new("RGB", (1440 * 2, max(mockup_sec.size[1], impl_sec.size[1])))
    combined.paste(mockup_sec, (0, 0))
    combined.paste(impl_sec, (1440, 0))
